								...
								<TABLE border=0 cellSpacing=0 cellPadding=1 width="100%">
									<TBODY>
										...
										<TR>
											...
											<form action="system/ad!commend" method="post">
														...
														<tr class=STYLE1 height=30 bgcolor=#ffffff>
															...
															<td>
																...
																<c:if test="${session_ad_commend_cycle==1&&session_ad_type==1}">
																		<input name="orderIndex" value="${ad.orderIndexTextOne}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==2&&session_ad_type==1}">
																		<input name="orderIndex" value="${ad.orderIndexTextTwo}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==3&&session_ad_type==1}">
																		<input name="orderIndex" value="${ad.orderIndexTextThree}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==4&&session_ad_type==1}">
																		<input name="orderIndex" value="${ad.orderIndexTextFour}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==5&&session_ad_type==1}">
																		<input name="orderIndex" value="${ad.orderIndexTextFive}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==1&&session_ad_type==2}">
																		<input name="orderIndex" value="${ad.orderIndexPicOne}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==2&&session_ad_type==2}">
																	    <input name="orderIndex" value="${ad.orderIndexPicTwo}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==3&&session_ad_type==2}">
																		<input name="orderIndex" value="${ad.orderIndexPicThree}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==4&&session_ad_type==2}">
																		<input name="orderIndex" value="${ad.orderIndexPicFour}"/> 
																	</c:if>
																	<c:if test="${session_ad_commend_cycle==5&&session_ad_type==2}">
																		<input name="orderIndex" value="${ad.orderIndexPicFive}"/> 
																	</c:if>
																&nbsp;&nbsp; <button type="submit">保存修改</button> 
																
																&nbsp;&nbsp; <img src="admin/tab/images/010.gif" alt="取消推荐" />&nbsp;<a href="system/ad!commendCancel?id=${ad.id}"type="button"  onclick="return confirm('您确定要取消推荐该广告吗?')"><font color="red">取消推荐</font></a>
															</td>
